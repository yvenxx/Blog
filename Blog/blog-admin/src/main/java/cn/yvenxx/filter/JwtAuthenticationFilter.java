package cn.yvenxx.filter;

import cn.yvenxx.R;
import cn.yvenxx.util.JWTUtil;
    ...
    */
    ...
    private void handleResponse(HttpServletRequest request, HttpServletResponse response, Authentication authResult, AuthenticationException failed) throws IOException, ServletException {
        ...
        if (authResult != null) {
            ...
            response.getWriter().write(mapper.writeValueAsString(R.ok("Bearer "+token,"登入成功")));
        } else {
            // 处理登入失败请求
            response.setStatus(HttpStatus.BAD_REQUEST.value());
            response.getWriter().write(mapper.writeValueAsString(R.error(400,"用户名或密码错误")));
        }
    }
}
